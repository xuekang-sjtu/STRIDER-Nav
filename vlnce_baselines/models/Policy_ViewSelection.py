...
class CMANet(Net):
    ...
    def forward(self, mode=None,
        ...
        if mode == 'language':
            ...
            # batch_size = 1
            batch_size = observations['instruction'].size(0) # observations['instruction'].size(0) = 1; observations['instruction'].size(1) = 200 
            ''' encoding rgb/depth at all directions ----------------------------- '''
            NUM_ANGLES = 120    # 120 angles 3 degrees each
            NUM_IMGS = 12
            NUM_CLASSES = 12    # 12 distances at each sector
            depth_batch = torch.zeros_like(observations['depth']).repeat(NUM_IMGS, 1, 1, 1) # (12, 256, 256, 1)
            rgb_batch = torch.zeros_like(observations['rgb']).repeat(NUM_IMGS, 1, 1, 1) # (12, 224, 224, 3)
            ...
            '''
                将输入图像的顺序反转成顺时针
                顺时针方向的单视图图像与全景图一致
                1st loop => ra_count = 0 => 'depth'在depth_batch[0]中
                2nd loop => ra_count = 11 => 'depth_30.0'在depth_batch[11]中
                3rd loop => ra_count = 10 => 'depth_60.0'在depth_batch[10]中
                4th loop => ra_count = 9 => 'depth_90.0'在depth_batch[9]中
                ...
            '''
            a_count = 0
            ...
            rgb_feats = torch.cat((
                rgb_embed_reshape[:,0:1,:], # 取出第一张照片
                torch.flip(rgb_embed_reshape[:,1:,:], [1]), # 在第二个维度上进行反转
            ), dim=1)
            ...
            for j in range(batch_size):
                ...
                angle_rad = 2*math.pi-angle_idxes.float()/120*2*math.pi # "2pi -" => 将角度从顺时针方向转换到逆时针方向
